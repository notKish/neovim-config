...
vim.opt.guicursor = {
  ...
  "i-ci-ve:ver25",        -- Insert mode: vertical bar (25% width)
  "r-cr:hor20",           -- Replace mode: horizontal bar (20% height)
  "o:hor50",              -- OperatorPending: horizontal bar (50% height)
  "a:blinkwait700-blinkoff400-blinkon250-Cursor/lCursor",
  "sm:block-blinkwait175-blinkoff150-blinkon175"
}

-- Terminal-specific cursor handling
vim.api.nvim_create_autocmd("TermEnter", {
  callback = function()
    vim.opt.guicursor = "a:ver25-blinkwait500"
  end
})
...
local function FloatingTerminal()
  ...
  if not terminal_state.buf or not vim.api.nvim_buf_is_valid(terminal_state.buf) then
    ...
    if not success then
      print("Error creating terminal buffer")
      return
    end
...
local function CloseFloatingTerminal()
  if terminal_state.is_open and terminal_state.win and vim.api.nvim_win_is_valid(terminal_state.win) then
    pcall(vim.api.nvim_win_close, terminal_state.win, false)
    terminal_state.is_open = false
  end
end
...
local function setup_shell_lsp()
  if vim.fn.executable('bash-language-server') == 0 then
    print("bash-language-server not found. Install with: npm i -g bash-language-server")
    return
  end
  
  local root_dir = find_root({'.git', 'Makefile', '.bashls'})
  vim.lsp.start({
    name = 'bashls',
